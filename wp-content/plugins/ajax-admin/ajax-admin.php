...
function ajax_admin_enqueue_scripts( $hook ) {
    ...
    // check if our page
    // ajax-admin-example is the slug.
    ...
    // localize script
    // the nonce is used by wp_localize_script to add some inline JavaScript.
    wp_localize_script( 'ajax-admin', 'ajax_admin', $script );

}
// Proper hook to use when adding javascript to admin pages.
...
// hook is used for ajax requests for users that are logged in
// wp_ajax tells wordpress the request is for logged in users
// the second part is the name of the action hook that is passed in the Post request, via the jquery post method
// admin_hook can be found in the javascript file

...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Web_Blog_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    web_blog_file_frame = wp.media.frames.web_blog_file_frame = wp.media({
      button: {
    ...
    web_blog_file_frame.on( 'select', function() {

      var state = web_blog_file_frame.state('author-info-insert-image');

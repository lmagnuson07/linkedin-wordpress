<?php
if ( ! class_exists( 'Web_Blog_Social_Widget' ) ) {
	/**
	 * Adds Web Blog Social Widget.
	 */
	class Web_Blog_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$web_blog_social_widget = array(
				'classname'   => 'widget adore-widget social-widget style-2',
				'description' => __( 'Retrive Social Widget', 'web-blog' ),
			);
			parent::__construct(
				'web_blog_social_widget',
				__( 'Web Blog: Social Widget', 'web-blog' ),
				$web_blog_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'social_icon_label' . '-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Icon Label %d :', 'web-blog' ), $i ); ?></label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Link %d :', 'web-blog' ), $i ); ?></label>
